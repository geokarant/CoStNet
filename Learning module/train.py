...
if __name__ == "__main__":
    ...
    model = utils.build_model(backbone, second_stage=(
        stage == 'second'), num_classes=num_classes, ckpt_pretrained=ckpt_pretrained).cuda()
    ...
    optim = utils.build_optim(model, optimizer_params,
                              scheduler_params, criterion_params)
    ...
    for epoch in range(n_epochs):
        ...
        if stage == 'first':
            ...
            train_metrics = utils.train_epoch_ce(
                loaders['train_features_loader'], model, criterion, optimizer, scaler, ema)
