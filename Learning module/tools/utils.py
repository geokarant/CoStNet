...
def build_transforms(second_stage):
    if second_stage:
        train_transforms = A.Compose([
            # A.Flip(),
            # A.Rotate(),
            #A.GaussNoise((100.0, 250.0), 0, 0.5), # I added this transformation
            A.Resize(244, 244),
            A.Normalize(),
            AT.ToTensorV2()
        ])
        valid_transforms = A.Compose(
            [A.Resize(224, 224), A.Normalize(), AT.ToTensorV2()])
...
def build_model(backbone, second_stage=False, num_classes=None, ckpt_pretrained=None):
    model = SupConModel(backbone=backbone,
                        second_stage=second_stage, num_classes=num_classes)

    if ckpt_pretrained:
        model.load_state_dict(torch.load(ckpt_pretrained)[
...
def build_optim(model, optimizer_params, scheduler_params, loss_params):
    ...
    optimizer = OPTIMIZERS[optimizer_params["name"]](
        model.parameters(), **optimizer_params["params"])
...
def compute_embeddings(loader, model, scaler):
    ...
    total_embeddings = np.zeros(
        (len(loader)*loader.batch_size, model.embed_dim))
    ...
    for idx, (images, labels) in enumerate(loader):
        ...
        if scaler:
            with torch.cuda.amp.autocast():
                embed = model(images)
                total_embeddings[idx * bsz: (idx + 1)
                                 * bsz] = embed.detach().cpu().numpy()
                total_labels[idx * bsz: (idx + 1) *
                             bsz] = labels.detach().numpy()
        else:
            embed = model(images)
            total_embeddings[idx * bsz: (idx + 1)
                             * bsz] = embed.detach().cpu().numpy()
...
def validation_ce(model, criterion, valid_loader, scaler):
    ...
    # note that it's okay to do len(loader) * bs, since drop_last=True is enabled
    y_pred, y_true = np.zeros(
        len(valid_loader)*valid_bs), np.zeros(len(valid_loader)*valid_bs)
    correct_samples = 0
    ...
    for batch_i, (data, target) in enumerate(valid_loader):
        with torch.no_grad():
            ...
            correct_samples += (
                target.detach().cpu().numpy() == np.argmax(
                    output.detach().cpu().numpy(), axis=1)
            ).sum()
            y_pred[batch_i * valid_bs: (batch_i + 1) * valid_bs] = np.argmax(
                output.detach().cpu().numpy(), axis=1)
            y_true[batch_i * valid_bs: (batch_i + 1) *
                   valid_bs] = target.detach().cpu().numpy()
    ...
    metrics = {"loss": valid_loss, "accuracy": accuracy_score,
               "f1_scores": f1_scores, 'f1_score_macro': f1_score_macro}
    return metrics


def copy_parameters_from_model(model):
    copy_of_model_parameters = [p.clone().detach()
